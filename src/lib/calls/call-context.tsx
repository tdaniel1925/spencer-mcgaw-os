...
export function CallProvider({ children }: { children: React.ReactNode }) {
  ...
  // Transform database row to CallRecord
  const transformDbCall = useCallback((dbCall: Record<string, unknown>): CallRecord => {
    ...
    return {
      ...
      status: (dbCall.status as CallStatus) || "new",
      createdAt: new Date(dbCall.created_at as string),
      updatedAt: new Date(dbCall.created_at as string),
      aiAnalysis: analysis ? {
        summary: (analysis.summary as string) || (dbCall.summary as string) || "Call recorded",
        keyPoints: (analysis.keyPoints as string[]) || [],
        category: (analysis.category as CallCategory) || (dbCall.intent as CallCategory) || "other",
        sentiment: (analysis.sentiment as "positive" | "negative" | "neutral" | "frustrated") || (dbCall.sentiment as "positive" | "negative" | "neutral" | "frustrated") || "neutral",
        urgency: (analysis.urgency as "low" | "medium" | "high" | "urgent") || "medium",
        extractedInfo: {
          reason: (analysis.summary as string) || (dbCall.summary as string) || "",
          questions: [],
          mentionedAmounts: [],
          mentionedDates: [],
        },
        suggestedActions: ((analysis.suggestedActions as string[]) || []).map((action, idx) => ({
          id: `action-${dbCall.id}-${idx}`,
        ...
        analyzedAt: new Date((metadata?.parsedAt as string) || (dbCall.created_at as string)),
      } : (dbCall.summary as string) ? {
        summary: dbCall.summary as string,
        keyPoints: [],
        category: (dbCall.intent as CallCategory) || "other",
        sentiment: (dbCall.sentiment as "positive" | "negative" | "neutral" | "frustrated") || "neutral",
        urgency: "medium" as const,
        extractedInfo: {
          reason: dbCall.summary as string,
          questions: [],
          mentionedAmounts: [],
          mentionedDates: [],
        },
        suggestedActions: [],
        confidence: 0.85,
        analyzedAt: new Date(dbCall.created_at as string),
      } : undefined,
    };
  }, []);

  // Create notification for a new call (with optional browser notification)
  const createNotificationForCall = useCallback((call: CallRecord) => {
    ...
    if (typeof window !== "undefined" && "Notification" in window && Notification.permission === "granted") {
      const browserNotif = new Notification(notification.title, {
        body: `${call.callerName || call.callerPhone}: ${notification.message}`,
        icon: "/favicon.ico",
        tag: `call-${call.id}`,
      });
  ...
  useEffect(() => {
    ...
    async function fetchInitialCalls() {
      try {
        ...
        if (data.success && data.calls) {
          // Transform API response (already formatted by API)
          ...
          // Mark all initial calls as seen (don't notify for old calls)
          transformedCalls.forEach((c: CallRecord) => seenCallIdsRef.current.add(c.id));
    ...
    // Subscribe to realtime changes on the calls table
    channelRef.current = supabase
      ...
      .on(
        ...
        (payload) => {
          ...
          if (!seenCallIdsRef.current.has(newCall.id)) {
            seenCallIdsRef.current.add(newCall.id);
  ...
  }, [transformDbCall, createNotificationForCall]);
