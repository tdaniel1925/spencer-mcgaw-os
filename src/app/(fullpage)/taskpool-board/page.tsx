...
export default function TaskPoolBoardPage() {
  ...
  // Get users who have tasks assigned
  const usersWithTasks = users.filter(user =>
    tasks.some(t => t.assigned_to === user.id && t.status !== "completed")
  );
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-xl font-bold tracking-tight">TaskPool Board</h1>
                <p className="text-xs text-muted-foreground">
                  Drag tasks between columns to assign or re-categorize
                </p>
              </div>
            </div>
          </div>
      ...
      <div className="flex-1 relative">
        ...
        >
          {loading ? (
            ...
            {/* Assigned Column - Groups by User */}
            <div
              className={cn(
                "w-80 flex-shrink-0 flex flex-col bg-card rounded-xl shadow-sm border overflow-hidden"
              )}
            >
              {/* Column Header */}
              <div className="flex items-center gap-2 px-3 py-2.5 border-b bg-muted/30 border-l-[3px] border-l-blue-500">
                    ...
                    >
                      ...
                      <div className="p-2 space-y-2 min-h-[60px]">
                        {userTasks.length === 0 ? (
                          ...
                          userTasks.map((task) => renderTaskCard(task, false))
                        )}
                      </div>
                    </div>
                  );
